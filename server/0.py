...
def demo_event(frame, data, parking, json_path):
    for car_id in data:
        car_bbox = data[car_id]["bbox"]
        for parking_id in parking:
            parking_boxes = parking[parking_id]
            dis = calculate_center_distance(parking_boxes, car_bbox)
            threshold = calculate_rectangle_diameter(car_bbox) / 3
            if data[car_id]["rec_score"] is None:
                continue
            if data[car_id]["rec_score"] < 0.8:
                continue
            if data[car_id]["parking"] == "out": 
                if dis < threshold:      # 说明车进停车位
                    response = in_event(data[car_id], parking_id, frame)
                    with open(json_path, "a", encoding="utf-8") as f_out:
                        f_out.write(json.dumps(response, ensure_ascii=False) + "\n")
                    data[car_id]["parking"] = parking_id
            elif data[car_id]["parking"] == parking_id:
                if dis > threshold:      # 说明车出停车位
                    response = out_event(data[car_id], parking_id, frame)
                    with open(json_path, "a", encoding="utf-8") as f_out:
                        f_out.write(json.dumps(response, ensure_ascii=False) + "\n")
                    data[car_id]["parking"] = "out"